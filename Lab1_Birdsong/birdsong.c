...
// compose mode
#define MAX_COMPOSE_KEYS 10
bool compose_mode = false;
int compose_sequence[MAX_COMPOSE_KEYS];
int compose_length = 0;
int compose_index = 0;
bool compose_playing = false;


// BEGIN AI-GENERATED CODE
// 0 to ~10 kHz, raw ADC reading runs 0-4095, stretch it before it becomes a phase increment.
// Recording and playback both call this
...
static PT_THREAD (protothread_playback(struct pt *pt))
{
   ...
   while(1) {
       ...
       if (compose_playing){
           while (compose_index < compose_length){
               play_key = compose_sequence[compose_index];


               for (sample = 0;
                    sample < record_length[play_key];
                    sample++) {
           ...
           case MAYBE_PRESSED:
               if (i == possible_key) {
                   ...
                   if (possible_key == 0){
                       ...
                       } else { //playback compose mode
                           compose_mode = false;
                           compose_playing = true;
                           compose_index = 0;
                           printf("compose mode off\n") ;
                       }
                   } else if (possible_key > 0 && possible_key < 10){
                       if (compose_mode) {
                           if (compose_length < MAX_COMPOSE_KEYS) {
                               compose_sequence[compose_length] = possible_key;
                               compose_length++;
                               printf("Added key %d to compose sequence\n", possible_key);
                           }
                       } else {
